
<footer class="pt-5 pr-5 mb-0 " style="background-color: #264653;">
    <div class="row mb-4" id="footer_menu">
        <div class="col-6 col-md-2 mb-3">
            <h5 class="text-white">A propos</h5>
            <ul class="nav flex-column">
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Qui sommes-nous ?</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Nos valeurs</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Notre équipes</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Plan du site</a></li>
            </ul>
        </div>

        <div class="col-6 col-md-2 mb-3">
            <h5 class="text-white">En savoir plus</h5>
            <ul class="nav flex-column">
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Presse</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Devenir partenaire</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Recrutement</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">FAQs</a></li>
            </ul>
        </div>

        <div class="col-6 col-md-2 mb-3">
            <h5 class="text-white">Informations légales</h5>
            <ul class="nav flex-column">
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">CGU</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Politique de confidentialité</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Mentions légales</a></li>
                <li class="nav-item mb-2"><a href="#" class="nav-link p-0 text-white">Cookies</a></li>
            </ul>
        </div>

        <div class="col-6 col-md-2 mb-3">
            <form>
                <h5 class="mb-3 text-white">Suivez-nous sur les réseaux !</h5>
                    <div class="d-flex flex-row  gap-2">
                        <a href="#"><img src="../assets/icons/facebook.png" alt="Facebook" style="width: 40px;"></a>
                        <a href="#"><img src="../assets/icons/instagram.png" alt="Instagram" style="width: 40px;"></a>
                        <a href="#"><img src="../assets/icons/linkedin.png" alt="Linkedin" style="width: 40px;"></a>
                    </div>
            </form>
        </div>
    </div>

    <div class="d-flex flex-column flex-sm-row justify-content-center mb-0 border-top">
        <p class="mt-3 text-white">© 2022 Company, Inc. All rights reserved.</p>
    </div>
</footer>
